...
use crate::core::ast::{ElementNode, Namespace, Node, RuntimeHelper};
...
fn get_namespace(tag: &str, parent: Option<&ElementNode>, root_ns: Namespace) -> Namespace {
    ...
    if let Some(parent) = parent {
        if ns == Namespace::MathMl {
            if parent.tag == "annotation-xml" {
                ...
                let html_encoding = parent.props.iter().any(|a| match a {
                    Node::Attribute(a) => {
                        a.name == "encoding"
                            && a.value.as_ref().is_some_and(|v| {
                                v.content == "text/html"
                                    || v.content == "application/xhtml+xml"
                            })
                    }
                    _ => false,
                });
